...
## Пайплайн разработки Этапа (проверенный портфельный, с Fable 5)
...
4. **Sonnet 5** (основной чат) — проверка тестового покрытия, написание/дополнение
   тестов, проверка работоспособности (`go test -race ./...`).
5. **Opus** через **Agent-тул** (`model: opus`) — независимое ревью `/code-review`
   на diff ветки.
6. Цикл исправлений — до **3 итераций** (Sonnet правит по замечаниям ревью).
